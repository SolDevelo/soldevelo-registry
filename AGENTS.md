...
This file is the single source of truth for every agent; there is no
...
5. Load the page and **measure** the preview height, then set `meta.height` to
   it. Do not guess: a wrong height leaves the card clipped or half empty.
...
- **Measure `meta.height`.** Nothing catches a wrong one.
